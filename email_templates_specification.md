# AMX Signs - Electric Noir Email Templates Specification

This document details the final Visual Guidelines and self-contained HTML layouts for all 8 transactional and lifecycle email templates. All emails are built with inline CSS styling for maximum compatibility across modern email clients and strictly adhere to the **"Electric Noir"** brand guidelines.

---

## Brand Theme Tokens (Inline CSS)
- **Background (Base):** `#000000` (Absolute Black)
- **Surface (Card):** `#0B0B0B` (Stealth Gray)
- **Primary CTA:** `#C6FF00` (Electric Lime)
- **Highlight/Pricing:** `#36F4A4` (Mint Glow)
- **Primary Text:** `#FFFFFF` (White)
- **Secondary Text:** `#A0A0A0` (Muted Gray)
- **Fonts:** `Outfit`, `Inter`, `Helvetica Neue`, `Arial`, `sans-serif`
- **Monospace Font:** `JetBrains Mono`, `Courier New`, `monospace`

---

## 1. Template 1A: Order Placed - Custom Signs
Sent when an order contains a custom neon sign. Highlights design specs, displays a live glowing neon preview, and establishes handcrafting timelines. There is no spelling check notice or review block.
...
## 2. Template 1B: Order Placed - Catalog Signs & Accessories
Sent when an order contains standard pre-designed catalog neon signs or storefront accessories. Emphasizes workshop handcrafting prep.
...
## 3. Template 2: Handcrafting Status Update
Sent once the design passes the initial file audit and reaches the physical workshop. Shows an active stage tracker.
...
## 4. Template 3: Quality Check (QC) Update
Sent when the sign completes assembly and is undergoing electrical stability, drop testing, and a continuous 24-hour glow test.
...
## 5. Template 4: Shipped
Sent when the product is packed in safety foam and picked up by the logistics carrier. Highlights the tracking URL.
...
## 6. Template 5: Delivered Status Update
Sent once the logistics carrier records a successful delivery. Encourages social engagement.
...
## 7. Template 6: Order Cancelled
Sent if an order is cancelled, either by customer request or system cancellation. Clarifies refund parameters.
...
## 8. Template 7: Password Reset (Set Your New Password)
A security template specifically branded with the heading "Set Your New Password".
...
## 9. Template 8: Abandoned Cart Recovery
Polled and dispatched 2 hours after a session goes inactive. Presents the discount code `COMEBACK5` with standard or customizer design details.
